...
EXPORT B_Education_7(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE230658 := __E_Education;
  EXPORT __ST177186_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST177186_Layout __ND1591152__Project(E_Education(__in,__cfg).Layout __PP230532) := TRANSFORM
    __BS230608 := __T(__PP230532.College_Characteristics_);
    SELF.Edu_Rec_Flag_ := EXISTS(__BS230608(__T(__OP2(__T(__PP230532.College_Characteristics_).File_Type_,IN,__CN(['M','C','H','O'])))));
    SELF := __PP230532;
  END;
  EXPORT __ENH_Education_7 := PROJECT(__EE230658,__ND1591152__Project(LEFT));
END;
