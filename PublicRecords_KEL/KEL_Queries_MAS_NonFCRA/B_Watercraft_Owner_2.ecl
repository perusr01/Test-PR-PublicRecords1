...
EXPORT B_Watercraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE2554959 := __ENH_Watercraft_Owner_3;
  EXPORT __ST164536_Layout := RECORD
  ...
  END;
  SHARED __ST164536_Layout __ND2555004__Project(B_Watercraft_Owner_3(__in,__cfg).__ST169241_Layout __PP2554960) := TRANSFORM
    __CC14091 := '-99997';
    SELF.Watercraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP2554960.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP2554960.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC14091)));
    SELF := __PP2554960;
  END;
  EXPORT __ENH_Watercraft_Owner_2 := PROJECT(__EE2554959,__ND2555004__Project(LEFT));
END;
