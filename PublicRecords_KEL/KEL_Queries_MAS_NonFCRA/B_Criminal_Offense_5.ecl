...
EXPORT B_Criminal_Offense_5(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE1642978 := __ENH_Criminal_Offense_6;
  EXPORT __ST172909_Layout := RECORD
  ...
  END;
  EXPORT __ST172862_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST172909_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST172862_Layout __ND1642898__Project(B_Criminal_Offense_6(__in,__cfg).__ST175289_Layout __PP1642346) := TRANSFORM
    __EE1642981 := __PP1642346.Offense_Charges_;
    __ST172909_Layout __ND1642856__Project(B_Criminal_Offense_6(__in,__cfg).__ST175336_Layout __PP1642507) := TRANSFORM
      __EE1642851 := __PP1642346.Offense_Charges_;
      SELF.Criminal_Date_ := KEL.Aggregates.MinNN(__EE1642851,KEL.Routines.CastStringToDate(__T(__EE1642851).Valid_Date_));
      SELF.Dismissed_Charges_ := __OR(IF(__T(__NT(__PP1642346.Court_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP1642346.Court_Disposition_Description_,=,__CN('DISMISSED')))),IF(__T(__NT(__PP1642346.Court_Additional_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP1642346.Court_Additional_Disposition_Description_,=,__CN('DISMISSED')))));
      SELF := __PP1642507;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE1642981,__ND1642856__Project(LEFT));
    SELF := __PP1642346;
  END;
  EXPORT __ENH_Criminal_Offense_5 := PROJECT(__EE1642978,__ND1642898__Project(LEFT));
END;
