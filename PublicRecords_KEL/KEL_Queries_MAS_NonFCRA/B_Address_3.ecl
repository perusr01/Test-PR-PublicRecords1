...
EXPORT B_Address_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  END;
  SHARED __ST164585_Layout __ND345441__Project(E_Address(__in,__cfg).Layout __PP341030) := TRANSFORM
    __EE341533 := __PP341030.A_D_V_O_Summary_;
    __BS341891 := __T(__EE341533);
    __EE341911 := __BN(TOPN(__BS341891(__NN(__T(__EE341533).A_D_V_O_Date_First_Seen_)),1, -__T(__T(__EE341533).A_D_V_O_Date_First_Seen_),__T(Vacancy_Indicator_),__T(Throw_Back_Indicator_),__T(Seasonal_Delivery_Indicator_),__T(Style_Code_),__T(Drop_Indicator_),__T(College_Indicator_),__T(Only_Way_To_Get_Mail_Indicator_),__T(Residential_Or_Business_Indicator_),__T(Do_Not_Deliver_Indicator_),__T(A_D_V_O_Date_Last_Seen_)),__NL(__EE341533));
    SELF.Bestchild_Advo_ := __EE341911;
    SELF := __PP341030;
  END;
  EXPORT __ENH_Address_3 := PROJECT(__EE341744,__ND345441__Project(LEFT));
END;
