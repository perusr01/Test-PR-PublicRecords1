...
EXPORT B_Person_Property_8(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE226719 := __E_Person_Property;
  SHARED __ST227255_Layout := RECORD
  ...
  END;
  SHARED __EE226649 := __E_Person_Property_Event;
  SHARED __EE1588906 := __EE226649(__NN(__EE226649.Event_) AND __NN(__EE226649.Subject_));
  SHARED __EE226627 := __E_Property_Event;
  SHARED __EE226848 := __EE226627(__T(__EE226627.Is_Deed_));
  SHARED __ST226885_Layout := RECORD
    KEL.typ.ntyp(E_Person().Typ) Subject_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC1588981(E_Person_Property_Event(__in,__cfg).Layout __EE1588906, E_Property_Event(__in,__cfg).Layout __EE226848) := __EEQP(__EE226848.UID,__EE1588906.Event_);
  __ST226885_Layout __JT1588981(E_Person_Property_Event(__in,__cfg).Layout __l, E_Property_Event(__in,__cfg).Layout __r) := TRANSFORM
    SELF.L_N_Fares_I_D__1_ := __r.L_N_Fares_I_D_;
    SELF.Data_Sources__1_ := __r.Data_Sources_;
    SELF := __l;
    SELF := __r;
  END;
  SHARED __EE1588982 := JOIN(__EE1588906,__EE226848,__JC1588981(LEFT,RIGHT),__JT1588981(LEFT,RIGHT),INNER,HASH);
  SHARED __ST227008_Layout := RECORD
    KEL.typ.ntyp(E_Person().Typ) Subject_;
    KEL.typ.ntyp(E_Property_Event().Typ) Event_;
    KEL.typ.nstr L_N_Fares_I_D_;
    KEL.typ.ndataset(E_Person_Property_Event(__in,__cfg).Party_Details_Layout) Party_Details_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __ST227008_Layout __JT1589084(__ST226885_Layout __l, E_Person_Property_Event(__in,__cfg).Party_Details_Layout __r) := TRANSFORM
    SELF.__RecordCount := __r.__RecordCount;
    SELF := __l;
    SELF := __r;
  END;
  SHARED __EE1589085 := NORMALIZE(__EE1588982,__T(LEFT.Party_Details_),__JT1589084(LEFT,RIGHT));
  SHARED __EE1589183 := __EE1589085(__T(__EE1589085.Party_Is_Seller_));
  __JC1589307(E_Person_Property(__in,__cfg).Layout __EE226719, __ST227008_Layout __EE1589183) := __EEQP(__EE226719.Prop_,__EE1589183.Prop_) AND __NNEQ(__EE1589183.Subject_,__EE226719.Subject_) AND __T(__AND(__EEQ(__EE226719.Prop_,__EE1589183.Prop_),__AND(__OP2(__EE1589183.Subject_,=,__EE226719.Subject_),__CN(__NN(__EE226719.Prop_)))));
  __JF1589307(__ST227008_Layout __EE1589183) := __NN(__EE1589183.Prop_) OR __NN(__EE1589183.Subject_);
  SHARED __EE1589332 := JOIN(__EE226719,__EE1589183,__JC1589307(LEFT,RIGHT),TRANSFORM(__ST227255_Layout,SELF:=LEFT,SELF.Person_Property_Event_:=__JF1589307(RIGHT)),HASH,LEFT OUTER,KEEP(1));
  EXPORT __ST179078_Layout := RECORD
    KEL.typ.ntyp(E_Person().Typ) Subject_;
    KEL.typ.ntyp(E_Property().Typ) Prop_;
    KEL.typ.nstr Primary_Range_;
    KEL.typ.nstr Predirectional_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  EXPORT __ENH_Person_Property_8 := PROJECT(__EE1589332,TRANSFORM(__ST179078_Layout,SELF.Property_Is_Sold_ := LEFT.Person_Property_Event_,SELF := LEFT));
END;
