...
EXPORT B_Input_P_I_I_5(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE1643067 := __ENH_Input_P_I_I_6;
  EXPORT __ST182651_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST182651_Layout __ND1643683__Project(B_Input_P_I_I_6(__in,__cfg).__ST182306_Layout __PP1643068) := TRANSFORM
    SELF.Addr_Not_Populated_ := FN_Compile(__cfg).FN_Is_Not_Enough_To_Clean(__ECAST(KEL.typ.nstr,__PP1643068.P___Inp_Addr_));
    SELF.City_State_Zip_Not_Populated_ := FN_Compile(__cfg).FN_City_State_Zip_Not_Populated_Check(__ECAST(KEL.typ.nstr,__PP1643068.P___Inp_Addr_City_),__ECAST(KEL.typ.nstr,__PP1643068.P___Inp_Addr_State_),__ECAST(KEL.typ.nstr,__PP1643068.P___Inp_Addr_Zip_));
    SELF.Name_Not_Populated_ := IF(FN_Compile(__cfg).FN_Name_Not_Populated_Check(__ECAST(KEL.typ.nstr,__PP1643068.P___Inp_Name_First_),__ECAST(KEL.typ.nstr,__PP1643068.P___Inp_Name_Mid_),__ECAST(KEL.typ.nstr,__PP1643068.P___Inp_Name_Last_)),TRUE,FALSE);
    SELF.P___Inp_Addr_Zip_Flag_Value_ := FN_Compile(__cfg).FN_Is_Echo_Populated(__ECAST(KEL.typ.nstr,__PP1643068.P___Inp_Addr_Zip_));
    __CC13664 := '-99999';
    SELF.P___Inp_Val_Name_Bogus_Flag_ := MAP(__T(__AND(__AND(__OP2(__PP1643068.Input_First_Name_Value_,=,__CN(__CC13664)),__OP2(__PP1643068.Input_Middle_Name_Value_,=,__CN(__CC13664))),__OP2(__PP1643068.Input_Last_Name_Value_,=,__CN(__CC13664))))=>__CC13664,__T(__OP2(FN_Compile(__cfg).FN__fn_Bogus_Names(__ECAST(KEL.typ.nstr,__FN1(KEL.Routines.ToUpperCase,__PP1643068.P___Inp_Name_First_)),__ECAST(KEL.typ.nstr,__FN1(KEL.Routines.ToUpperCase,__PP1643068.Input_Middle_Name_Value_)),__ECAST(KEL.typ.nstr,__FN1(KEL.Routines.ToUpperCase,__PP1643068.Input_Last_Name_Value_))),=,__CAST(KEL.typ.str,__CN(1))))=>(KEL.typ.str)1,(KEL.typ.str)0);
    SELF := __PP1643068;
  END;
  EXPORT __ENH_Input_P_I_I_5 := PROJECT(__EE1643067,__ND1643683__Project(LEFT));
END;
