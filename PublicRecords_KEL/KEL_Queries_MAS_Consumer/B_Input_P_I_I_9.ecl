...
EXPORT B_Input_P_I_I_9(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE158041 := __E_Input_P_I_I;
  EXPORT __ST152793_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST152793_Layout __ND287472__Project(E_Input_P_I_I(__in,__cfg).Layout __PP157361) := TRANSFORM
    SELF.P___Inp_Addr_ := __OP2(IF(__T(__NT(__PP157361.P___Inp_Addr_Line1_)),__ECAST(KEL.typ.nstr,__CN('')),__ECAST(KEL.typ.nstr,__OP2(__PP157361.P___Inp_Addr_Line1_,+,__CN(' ')))),+,IF(__T(__NT(__PP157361.P___Inp_Addr_Line2_)),__ECAST(KEL.typ.nstr,__CN('')),__ECAST(KEL.typ.nstr,__PP157361.P___Inp_Addr_Line2_)));
    SELF := __PP157361;
  END;
  EXPORT __ENH_Input_P_I_I_9 := PROJECT(__EE158041,__ND287472__Project(LEFT));
END;
