...
EXPORT B_Address_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE174612 := __E_Address;
  EXPORT __ST148197_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST148197_Layout __ND178309__Project(E_Address(__in,__cfg).Layout __PP173898) := TRANSFORM
    __EE174401 := __PP173898.A_D_V_O_Summary_;
    __BS174759 := __T(__EE174401);
    __EE174779 := __BN(TOPN(__BS174759(__NN(__T(__EE174401).A_D_V_O_Date_First_Seen_)),1, -__T(__T(__EE174401).A_D_V_O_Date_First_Seen_),__T(Vacancy_Indicator_),__T(Throw_Back_Indicator_),__T(Seasonal_Delivery_Indicator_),__T(Style_Code_),__T(Drop_Indicator_),__T(College_Indicator_),__T(Only_Way_To_Get_Mail_Indicator_),__T(Residential_Or_Business_Indicator_),__T(Do_Not_Deliver_Indicator_),__T(A_D_V_O_Date_Last_Seen_)),__NL(__EE174401));
    SELF.Bestchild_Advo_ := __EE174779;
    SELF := __PP173898;
  END;
  EXPORT __ENH_Address_3 := PROJECT(__EE174612,__ND178309__Project(LEFT));
END;
