...
EXPORT B_Property_Event_5(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE420730 := __E_Property_Event;
  EXPORT __ST206488_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST206488_Layout __ND1988410__Project(E_Property_Event(__in,__cfg).Layout __PP420388) := TRANSFORM
    ...
    SELF.Is_Current_Assessment_Record_ := __AND(__PP420388.Is_Assessment_,__OP2(FN_Compile(__cfg).FN_A_B_S_Y_E_A_R_S_B_E_T_W_E_E_N(__ECAST(KEL.typ.nkdate,KEL.era.ToDate(__PP420388.Date_First_Seen_)),__ECAST(KEL.typ.nkdate,__CC13407)),<=,__CN(1)));
    SELF := __PP420388;
  END;
  EXPORT __ENH_Property_Event_5 := PROJECT(__EE420730,__ND1988410__Project(LEFT));
END;
