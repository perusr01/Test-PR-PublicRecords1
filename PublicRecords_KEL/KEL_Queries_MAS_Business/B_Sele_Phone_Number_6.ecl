...
EXPORT B_Sele_Phone_Number_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE361435 := __E_Sele_Phone_Number;
  SHARED __EE1939630 := __ENH_Business_Sele_7;
  ...
  END;
  __JC1939636(E_Sele_Phone_Number(__in,__cfg).Layout __EE361435, B_Business_Sele_7(__in,__cfg).__ST211324_Layout __EE1939630) := __EEQP(__EE361435.Legal_,__EE1939630.UID);
  __ST361579_Layout __JT1939636(E_Sele_Phone_Number(__in,__cfg).Layout __l, B_Business_Sele_7(__in,__cfg).__ST211324_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE1939637 := JOIN(__EE361435,__EE1939630,__JC1939636(LEFT,RIGHT),__JT1939636(LEFT,RIGHT),LEFT OUTER,HASH);
  EXPORT __ST210285_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  EXPORT __ST210247_Layout := RECORD
    KEL.typ.ntyp(E_Business_Sele().Typ) Legal_;
    KEL.typ.ntyp(E_Phone().Typ) Phone_Number_;
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).Best_Phone_Details_Layout) Best_Phone_Details_;
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).Phone_Details_Layout) Phone_Details_;
    ...
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).S_I_C_Codes_Layout) S_I_C_Codes_;
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).N_A_I_C_S_Codes_Layout) N_A_I_C_S_Codes_;
    KEL.typ.ndataset(__ST210285_Layout) Data_Sources_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST210247_Layout __ND1939996__Project(__ST361579_Layout __PP1939638) := TRANSFORM
    __EE1940045 := __PP1939638.Data_Sources_;
    __ST210285_Layout __ND1940050__Project(E_Sele_Phone_Number(__in,__cfg).Data_Sources_Layout __PP1940046) := TRANSFORM
      SELF.My_Date_First_Seen_ := KEL.era.ToDate(__PP1940046.Date_First_Seen_);
      SELF.My_Date_Last_Seen_ := KEL.era.ToDate(__PP1940046.Date_Last_Seen_);
      SELF.Translated_Source_ := FN_Compile(__cfg).FN_Source_Group(__ECAST(KEL.typ.nstr,__PP1940046.Source_));
      SELF := __PP1940046;
    END;
    SELF.Data_Sources_ := __PROJECT(__EE1940045,__ND1940050__Project(LEFT));
    SELF.Input_Phone_Match_ := FN_Compile(__cfg).FN_Is_Phone_Match(__ECAST(KEL.typ.nstr,__ECAST(KEL.typ.nstr,__PP1939638.Phone_Number_)),__ECAST(KEL.typ.nstr,__PP1939638.B___Inp_Cln_Phone_));
    SELF := __PP1939638;
  END;
  EXPORT __ENH_Sele_Phone_Number_6 := PROJECT(__EE1939637,__ND1939996__Project(LEFT));
END;
