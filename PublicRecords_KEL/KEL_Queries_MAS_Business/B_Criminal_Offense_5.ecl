...
EXPORT B_Criminal_Offense_5(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE1985768 := __ENH_Criminal_Offense_6;
  EXPORT __ST205277_Layout := RECORD
  ...
  END;
  EXPORT __ST205230_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST205277_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST205230_Layout __ND1985688__Project(B_Criminal_Offense_6(__in,__cfg).__ST209010_Layout __PP1985144) := TRANSFORM
    __EE1985771 := __PP1985144.Offense_Charges_;
    __ST205277_Layout __ND1985649__Project(B_Criminal_Offense_6(__in,__cfg).__ST209057_Layout __PP1985305) := TRANSFORM
      __EE1985644 := __PP1985144.Offense_Charges_;
      SELF.Criminal_Date_ := KEL.Aggregates.MinNN(__EE1985644,KEL.Routines.CastStringToDate(__T(__EE1985644).Valid_Date_));
      SELF.Dismissed_Charges_ := __OR(IF(__T(__NT(__PP1985144.Court_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP1985144.Court_Disposition_Description_,=,__CN('DISMISSED')))),IF(__T(__NT(__PP1985144.Court_Additional_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP1985144.Court_Additional_Disposition_Description_,=,__CN('DISMISSED')))));
      SELF := __PP1985305;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE1985771,__ND1985649__Project(LEFT));
    SELF := __PP1985144;
  END;
  EXPORT __ENH_Criminal_Offense_5 := PROJECT(__EE1985768,__ND1985688__Project(LEFT));
END;
