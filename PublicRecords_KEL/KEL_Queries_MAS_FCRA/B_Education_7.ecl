...
EXPORT B_Education_7(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE214052 := __E_Education;
  EXPORT __ST168064_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST168064_Layout __ND1082771__Project(E_Education(__in,__cfg).Layout __PP213926) := TRANSFORM
    __BS214002 := __T(__PP213926.College_Characteristics_);
    SELF.Edu_Rec_Flag_ := EXISTS(__BS214002(__T(__OP2(__T(__PP213926.College_Characteristics_).File_Type_,IN,__CN(['M','C','H','O'])))));
    SELF := __PP213926;
  END;
  EXPORT __ENH_Education_7 := PROJECT(__EE214052,__ND1082771__Project(LEFT));
END;
