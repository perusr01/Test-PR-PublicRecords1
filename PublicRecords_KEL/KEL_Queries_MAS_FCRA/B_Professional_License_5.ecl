...
EXPORT B_Professional_License_5(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE1130019 := __E_Professional_License;
  EXPORT __ST166484_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST166484_Layout __ND1129946__Project(E_Professional_License(__in,__cfg).Layout __PP257167) := TRANSFORM
    __BS257316 := __T(__PP257167.License_Dates_);
    ...
    SELF.B_U_I_L_D___D_A_T_E_ := KEL.Routines.MinN(IF(EXISTS(__BS257316(__T(__OP2(__T(__PP257167.License_Dates_).Source_,=,__CN('PL'))))),__ECAST(KEL.typ.nkdate,__CC13391),__ECAST(KEL.typ.nkdate,__CC13399)),__CN(__cfg.CurrentDate));
    __EE1129941 := __PP257167.License_Dates_;
    SELF.Max_Expire_Date_ := KEL.Aggregates.MaxNN(__EE1129941,__T(__EE1129941).Date_Of_Expiration_);
    __EE1129966 := __PP257167.License_Dates_;
    SELF.Max_Issue_Date_ := KEL.Aggregates.MaxNN(__EE1129966,IF(__T(__OP2(__T(__EE1129966).Original_Date_Of_Issuance_,<,KEL.Routines.CastStringToDate(__CN('19000101')))),__ECAST(KEL.typ.nkdate,KEL.Routines.CastStringToDate(__CN(''))),__ECAST(KEL.typ.nkdate,__T(__EE1129966).Original_Date_Of_Issuance_)));
    SELF := __PP257167;
  END;
  EXPORT __ENH_Professional_License_5 := PROJECT(__EE1130019,__ND1129946__Project(LEFT));
END;
