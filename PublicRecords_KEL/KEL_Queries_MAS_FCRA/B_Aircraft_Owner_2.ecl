...
EXPORT B_Aircraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE1263561 := __ENH_Aircraft_Owner_3;
  EXPORT __ST155437_Layout := RECORD
  ...
  END;
  SHARED __ST155437_Layout __ND1263621__Project(B_Aircraft_Owner_3(__in,__cfg).__ST159311_Layout __PP1263562) := TRANSFORM
    __CC14076 := '-99997';
    SELF.Aircraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP1263562.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP1263562.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC14076)));
    SELF := __PP1263562;
  END;
  EXPORT __ENH_Aircraft_Owner_2 := PROJECT(__EE1263561,__ND1263621__Project(LEFT));
END;
