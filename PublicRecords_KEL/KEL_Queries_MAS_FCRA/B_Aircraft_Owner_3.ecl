...
EXPORT B_Aircraft_Owner_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE285069 := __E_Aircraft_Owner;
  EXPORT __ST159311_Layout := RECORD
  ...
  END;
  SHARED __ST159311_Layout __ND285081__Project(E_Aircraft_Owner(__in,__cfg).Layout __PP285007) := TRANSFORM
    ...
    SELF.Date_First_Seen_Capped_ := IF(__T(__OP2(KEL.era.ToDate(__PP285007.Date_First_Seen_),>,__CC13279)),__ECAST(KEL.typ.nkdate,__CC13279),__ECAST(KEL.typ.nkdate,KEL.era.ToDate(__PP285007.Date_First_Seen_)));
    SELF := __PP285007;
  END;
  EXPORT __ENH_Aircraft_Owner_3 := PROJECT(__EE285069,__ND285081__Project(LEFT));
END;
