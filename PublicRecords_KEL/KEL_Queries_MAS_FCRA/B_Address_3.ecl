...
EXPORT B_Address_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE281003 := __E_Address;
  EXPORT __ST159133_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST159133_Layout __ND284700__Project(E_Address(__in,__cfg).Layout __PP280289) := TRANSFORM
    __EE280792 := __PP280289.A_D_V_O_Summary_;
    __BS281150 := __T(__EE280792);
    __EE281170 := __BN(TOPN(__BS281150(__NN(__T(__EE280792).A_D_V_O_Date_First_Seen_)),1, -__T(__T(__EE280792).A_D_V_O_Date_First_Seen_),__T(Vacancy_Indicator_),__T(Throw_Back_Indicator_),__T(Seasonal_Delivery_Indicator_),__T(Style_Code_),__T(Drop_Indicator_),__T(College_Indicator_),__T(Only_Way_To_Get_Mail_Indicator_),__T(Residential_Or_Business_Indicator_),__T(Do_Not_Deliver_Indicator_),__T(A_D_V_O_Date_Last_Seen_)),__NL(__EE280792));
    SELF.Bestchild_Advo_ := __EE281170;
    SELF := __PP280289;
  END;
  EXPORT __ENH_Address_3 := PROJECT(__EE281003,__ND284700__Project(LEFT));
END;
