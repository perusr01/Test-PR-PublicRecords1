...
EXPORT B_Person_7(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE1084622 := __ENH_Person_8;
  SHARED __EE1084624 := __ENH_Input_P_I_I_8;
  SHARED __ST216411_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(B_Person_8(__in,__cfg).__ST207744_Layout) All_Lien_Data_;
  ...
  END;
  __JC1084633(B_Person_8(__in,__cfg).__ST169244_Layout __EE1084622, B_Input_P_I_I_8(__in,__cfg).__ST168942_Layout __EE1084624) := __EEQP(__EE1084622.P_I_I_,__EE1084624.UID);
  __ST216411_Layout __JT1084633(B_Person_8(__in,__cfg).__ST169244_Layout __l, B_Input_P_I_I_8(__in,__cfg).__ST168942_Layout __r) := TRANSFORM
    SELF.U_I_D__1_ := __r.UID;
    SELF := __l;
    SELF := __r;
  END;
  SHARED __EE1084857 := JOIN(__EE1084622,__EE1084624,__JC1084633(LEFT,RIGHT),__JT1084633(LEFT,RIGHT),LEFT OUTER,SMART);
  EXPORT __ST173924_Layout := RECORD
    KEL.typ.nstr Title_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  EXPORT __ST215391_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  EXPORT __ST173919_Layout := RECORD
    KEL.typ.nuid UID;
    KEL.typ.nstr Lex_I_D_Segment_;
    KEL.typ.nstr Lex_I_D_Segment2_;
    KEL.typ.ndataset(__ST173924_Layout) Full_Name_;
    KEL.typ.ndataset(E_Person(__in,__cfg).Reported_Dates_Of_Birth_Layout) Reported_Dates_Of_Birth_;
    KEL.typ.ndataset(E_Person(__in,__cfg).Reported_Dates_Of_Death_Layout) Reported_Dates_Of_Death_;
    KEL.typ.ndataset(E_Person(__in,__cfg).Data_Sources_Layout) Data_Sources_;
    KEL.typ.ndataset(E_Person(__in,__cfg).Conceal_Carry_Layout) Conceal_Carry_;
    ...
    KEL.typ.ndataset(E_Person(__in,__cfg).Consumer_Statement_Flags_Layout) Consumer_Statement_Flags_;
    KEL.typ.ndataset(E_Person(__in,__cfg).Thrive_Layout) Thrive_;
    KEL.typ.ndataset(__ST215391_Layout) All_Lien_Data_;
    KEL.typ.ntyp(E_Input_P_I_I().Typ) P_I_I_;
    KEL.typ.nstr P___Inp_Cln_Name_First_Raw_;
    KEL.typ.nstr P___Inp_Cln_Name_Last_Raw_;
    KEL.typ.nstr P___Inp_Cln_S_S_N_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST173919_Layout __ND1084533__Project(__ST216411_Layout __PP1083107) := TRANSFORM
    __EE1084860 := __PP1083107.Full_Name_;
    __ST173924_Layout __ND1083410__Project(E_Person(__in,__cfg).Full_Name_Layout __PP1083406) := TRANSFORM
      SELF.Translated_Source_Code_ := FN_Compile(__cfg).FN_Consumer_Source_Group(__ECAST(KEL.typ.nstr,__PP1083406.Source_));
      SELF.Verified_First_Name_ := FN_Compile(__cfg).FN_Is_First_Name_Match(__ECAST(KEL.typ.nstr,__PP1083406.First_Name_),__ECAST(KEL.typ.nstr,__PP1083107.P___Inp_Cln_Name_First_Raw_));
      SELF.Verified_Last_Name_ := FN_Compile(__cfg).FN_Is_Last_Name_Match(__ECAST(KEL.typ.nstr,__PP1083406.Last_Name_),__ECAST(KEL.typ.nstr,__PP1083107.P___Inp_Cln_Name_Last_Raw_));
      SELF := __PP1083406;
    END;
    SELF.Full_Name_ := __PROJECT(__EE1084860,__ND1083410__Project(LEFT));
    __EE1084531 := __PP1083107.All_Lien_Data_;
    __ST215391_Layout __ND1084437__Project(B_Person_8(__in,__cfg).__ST207744_Layout __PP1083483) := TRANSFORM
      ...
      SELF.Is_Landlord_Tenant_Dispute_ := __AND(__OR(__CN(__PP1083483.Landlord_Tenant_Dispute_Flag_ = TRUE),__OP2(__PP1083483.Filing_Type_Description_,IN,__CN(__CC33958))),__NOT(__OP2(__PP1083483.Filing_Type_Description_,IN,__CN(__CC33971))));
      SELF.Is_Over_All_Lien_Judgment_ := __OR(__PP1083483.Is_Over_All_Judgment_,__PP1083483.Is_Over_All_Lien_);
      __CC33952 := 2556;
      SELF.Seen___In___Seven___Years_ := __OP2(__PP1083483.Age_In_Days_,<=,__CN(__CC33952));
      SELF := __PP1083483;
    END;
    SELF.All_Lien_Data_ := __PROJECT(__EE1084531,__ND1084437__Project(LEFT));
    SELF.P___Inp_Cln_S_S_N_ := __PP1083107.Input_S_S_N_Clean_Value_;
    __BS1084543 := __T(__PP1083107.Data_Sources_);
    SELF.P___Lex_I_D_Seen_Flag_ := IF(EXISTS(__BS1084543(__T(__T(__PP1083107.Data_Sources_).Header_Hit_Flag_))),'1','0');
    SELF := __PP1083107;
  END;
  EXPORT __ENH_Person_7 := PROJECT(__EE1084857,__ND1084533__Project(LEFT));
END;
