...
EXPORT B_Property_Event_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE1107688 := __ENH_Property_Event_7;
  EXPORT __ST167756_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST167756_Layout __ND1108042__Project(B_Property_Event_7(__in,__cfg).__ST168635_Layout __PP1107689) := TRANSFORM
    ...
    SELF.Is_Current_Assessment_Record_ := __AND(__PP1107689.Is_Assessment_,__OP2(FN_Compile(__cfg).FN_A_B_S_Y_E_A_R_S_B_E_T_W_E_E_N(__ECAST(KEL.typ.nkdate,KEL.era.ToDate(__PP1107689.Date_First_Seen_)),__ECAST(KEL.typ.nkdate,__CC13407)),<=,__CN(1)));
    __CC72707 := ['G','T','WD','DE','GD','SW','TD','X','BS','SD','CP','LW','SV','CH','OT','PA','PR','JT','ID','EX','LD','CS','RD','CD','FD','LT','CM','AD','AG','LH','D','PD','LC','AS','RC','CO','DG','MD','DC','RA'];
    SELF.Property_Purchase_Record_ := __OR(__OP2(__PP1107689.Document_Type_Code_,IN,__CN(__CC72707)),__OP2(__PP1107689.Sale_Price_,>,__CN(0)));
    SELF := __PP1107689;
  END;
  EXPORT __ENH_Property_Event_6 := PROJECT(__EE1107688,__ND1108042__Project(LEFT));
END;
