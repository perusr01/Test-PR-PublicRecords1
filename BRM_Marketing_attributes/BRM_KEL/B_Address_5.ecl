...
EXPORT B_Address_5(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE265675 := __E_Address;
  EXPORT __ST161623_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST161623_Layout __ND269372__Project(E_Address(__in,__cfg).Layout __PP264961) := TRANSFORM
    __EE265464 := __PP264961.A_D_V_O_Summary_;
    __BS265822 := __T(__EE265464);
    __EE265842 := __BN(TOPN(__BS265822(__NN(__T(__EE265464).A_D_V_O_Date_First_Seen_)),1, -__T(__T(__EE265464).A_D_V_O_Date_First_Seen_),__T(Vacancy_Indicator_),__T(Throw_Back_Indicator_),__T(Seasonal_Delivery_Indicator_),__T(Style_Code_),__T(Drop_Indicator_),__T(College_Indicator_),__T(Only_Way_To_Get_Mail_Indicator_),__T(Residential_Or_Business_Indicator_),__T(Do_Not_Deliver_Indicator_),__T(A_D_V_O_Date_Last_Seen_)),__NL(__EE265464));
    SELF.Bestchild_Advo_ := __EE265842;
    SELF := __PP264961;
  END;
  EXPORT __ENH_Address_5 := PROJECT(__EE265675,__ND269372__Project(LEFT));
END;
