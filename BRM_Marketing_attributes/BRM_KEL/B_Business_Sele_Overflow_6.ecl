...
EXPORT B_Business_Sele_Overflow_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE256514 := __E_Business_Sele_Overflow;
  EXPORT __ST165346_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST165346_Layout __ND846615__Project(E_Business_Sele_Overflow(__in,__cfg).Layout __PP256046) := TRANSFORM
    __EE256357 := __PP256046.S_O_S_Incorporation_Details_;
    __BS257891 := __T(__EE256357);
    __EE846613 := __BS257891(__T(__AND(__AND(__NOT(__NT(__T(__EE256357).S_O_S_Incorporation_State_)),__OP2(__T(__EE256357).S_O_S_Incorporation_State_,<>,__CN(''))),__OP2(__T(__EE256357).S_O_S_Foreign_Domestic_Indicator_,=,__CN('D')))));
    SELF.S_O_S_Domestic_Filing_ := __CN(__EE846613);
    __EE256388 := __PP256046.S_O_S_Incorporation_Details_;
    __BS258093 := __T(__EE256388);
    __EE846623 := __BS258093(__T(__AND(__AND(__NOT(__NT(__T(__EE256388).S_O_S_Foreign_State_Code_)),__OP2(__T(__EE256388).S_O_S_Foreign_State_Code_,<>,__CN(''))),__OP2(__T(__EE256388).S_O_S_Foreign_Domestic_Indicator_,=,__CN('F')))));
    SELF.S_O_S_Foreign_Filing_ := __CN(__EE846623);
    SELF := __PP256046;
  END;
  EXPORT __ENH_Business_Sele_Overflow_6 := PROJECT(__EE256514,__ND846615__Project(LEFT));
END;
