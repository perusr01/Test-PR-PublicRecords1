...
EXPORT B_Bankruptcy_5(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE852964 := __ENH_Bankruptcy_6;
  EXPORT __ST269702_Layout := RECORD
  ...
  END;
  EXPORT __ST161831_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST269702_Layout) Best_Child_Record_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST161831_Layout __ND852921__Project(B_Bankruptcy_6(__in,__cfg).__ST164510_Layout __PP852586) := TRANSFORM
    __EE852919 := __PP852586.Best_Child_Record_;
    __ST269702_Layout __ND852854__Project(B_Bankruptcy_7(__in,__cfg).__ST166538_Layout __PP852644) := TRANSFORM
      ...
      SELF.Filing_Age_In_Days_ := FN_Compile(__cfg).FN_A_B_S_D_A_Y_S_B_E_T_W_E_E_N(__ECAST(KEL.typ.nkdate,__PP852644.Date_Filed_),__ECAST(KEL.typ.nkdate,__CC13287));
      __BS852711 := __T(__PP852586.Case_Details_);
      SELF.Is_Bankruptcy_ := __AND(__PP852586.Has_Case_Number_,__CN(EXISTS(__BS852711(__T(__OP2(__CAST(KEL.typ.str,__T(__PP852586.Case_Details_).Case_I_D_),<>,__CN('')))))));
      SELF := __PP852644;
    END;
    SELF.Best_Child_Record_ := __FILTER(__PROJECT(__EE852919,__ND852854__Project(LEFT)),__NN(Source_Description_) OR __NN(Original_Chapter_) OR __NN(Filing_Type_) OR __NN(Business_Flag_) OR __NN(Corporate_Flag_) OR __NN(Discharged_Date_) OR __NN(Disposition_) OR __NN(Debtor_Type_) OR __NN(Debtor_Sequence_) OR __NN(Disposition_Type_) OR __NN(Disposition_Reason_) OR __NN(Disposition_Type_Description_) OR __NN(Name_Type_) OR __NN(Screen_Description_) OR __NN(Decoded_Description_) OR __NN(Date_Filed_) OR __NN(Record_Type_) OR __NN(Last_Status_Update_) OR __NN(Filing_Age_In_Days_) OR __NN(Is_Bankruptcy_));
    SELF := __PP852586;
  END;
  EXPORT __ENH_Bankruptcy_5 := PROJECT(__EE852964,__ND852921__Project(LEFT));
END;
